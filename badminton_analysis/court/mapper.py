...
class CourtMapper:
    def __init__(self, image_court_corners, court_dimensions=(6.1, 13.4)):
        ...
        # Mapping follows auto-detection preview convention:
        #   corners[0]=top-left(image) → court far-left
        #   corners[1]=top-right(image) → court far-right
        #   corners[2]=bottom-right(image) → court near-right
        #   corners[3]=bottom-left(image) → court near-left
        court_points = np.array([
            [0, court_dimensions[1]], [court_dimensions[0], court_dimensions[1]],
            [court_dimensions[0], 0], [0, 0]
        ], dtype=np.float32)
